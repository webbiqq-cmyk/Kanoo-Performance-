...
management-demo/      Internal "Performance Studio" management/sales demo — see management-demo/README.md
```

## Management demo

`management-demo/` is a separate React + TypeScript + Tailwind app (Command Centre, Performance
Studio, Calibration Lab, Workshop Operations) linked from the main nav as **Management Demo**. It
has its own build step and README — see `management-demo/README.md`. The committed
`management-demo/dist/` is what the nav link opens directly; rebuild it after changing the app's
source with `cd management-demo && npm install && npm run build`.
